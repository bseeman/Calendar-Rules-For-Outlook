...
<tr>
                        <td><p>Matter:</p></td>
                        <td>
                         <input type="text" name="cmbMatter" id="cmbMatter" value="<?php echo stripslashes($_POST['cmbMatter']);?>" /> 
</td> 
...
<?php	
	...
	echo "<tr><td valign=\"top\"><div>";
		...
		$justtime=substr($response[CalendarRuleEvent][EventDate],11,8);
		if ($justtime=="00:00:00") {
			$justtime= "";
		}
			?>
    <ul class="triggersnav">
		...
		echo $montharray[$mo]." ".substr($justdate,8,2).", ".substr($justdate,0,4) ." ".$justtime." - ";
		
		if ($_POST['cmbMatter'] != "" ) {
			echo "(".stripslashes($_POST['cmbMatter']).") ";
		} 
...
foreach ($response as $Event) { ?>
	...
	$mo=substr($justdate,5,2);
		...
		$justtime=substr($Event[CalendarRuleEvent][EventDate],11,8);
		if ($justtime=="00:00:00") {
			$justtime= "";
		}
